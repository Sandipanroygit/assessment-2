...
type UploadCurriculumViewProps = {
  embedded?: boolean;
  onDone?: () => void;
};

export function UploadCurriculumView({ embedded = false, onDone }: UploadCurriculumViewProps) {
  const router = useRouter();
  const [grade, setGrade] = useState(grades[0]);
  const [subject, setSubject] = useState(subjects[0]);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [judgingLogic, setJudgingLogic] = useState("");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("curriculum_modules").insert({
        ...
        module: "Drone Module",
        description,
        judging_logic: judgingLogic.trim() || null,
        asset_urls: assets,
        published: true,
      });

      if (error) {
        if (/judging_logic/i.test(error.message || "")) {
          setStatus(
            "Upload blocked: judging logic column is missing in Supabase. Apply `supabase/schema.sql` and retry.",
          );
          return;
        }
  ...
  const rootClassName = embedded ? "space-y-6" : "section-padding space-y-8";
  ...
  return (
    <section className={rootClassName}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-300 text-sm mt-2">
            Choose grade and subject, then add title, description, judging logic, video, Python code, and the user manual.
          </p>
        </div>
        {!embedded && (
          <Link
      ...
      <form onSubmit={handleSubmit} className="glass-panel rounded-3xl p-6 space-y-4 border border-white/20">
        ...
        <label className="block text-sm text-slate-300 space-y-2">
          AI judging logic (for reports)
          <textarea
            value={judgingLogic}
            onChange={(e) => setJudgingLogic(e.target.value)}
            className="w-full rounded-xl border border-slate-400/60 bg-white/5 px-3 py-2 text-white focus:border-accent focus:outline-none"
            rows={4}
            placeholder="Example: Score higher if the plot follows an inverse pressure-vs-altitude trend, with consistent sampling and minimal spikes."
          />
          <p className="text-xs text-slate-400">
            This logic will be sent to AI report generation for this activity.
          </p>
        </label>
